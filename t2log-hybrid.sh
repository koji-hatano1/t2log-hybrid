...
#  METHOD:    Hatano Skull Stripping Method - Hybrid (v4.10)
#  STRATEGY:  Hybrid SynthStrip Mask via T2w-based Log-Normal Thresholding
#             & T1w-based Squared-Space Thresholding for Ventral Refinement
...
GLOBAL_LOG="hss-t2lh_v4.10_global_${TIMESTAMP}.log"
...
log_info "=== Hatano Skull Stripping Method - Hybrid v4.10 Started ==="

for SESSION in ${Subjlist}; do
    # --- Subject-specific Logging Setup ---
    SUBJ_LOG="${BASE_PATH}/hss-t2lh_v4.10_${SESSION}_${TIMESTAMP}.log"
    SUBJ_ERR="${BASE_PATH}/hss-t2lh_v4.10_${SESSION}_${TIMESTAMP}.err"
    ...
    if [ -f "${T1wFolder}/T2w_acpc_dc_restore.nii.gz" ]; then
        ...
        if mri_synthstrip \
            ...
            # --- 2. T1w Processing (Squared Space) ---
            ...
            # --- 3. Batch Backup Processing ---
            log_info "  Step B-2: Backing up original HCP/FS outputs..."

            for msk in "$MASK" "$FS_MASK" "$MNI_MASK"; do
                if [ -f "$msk" ]; then
            ...
            # --- 4. Building Hybrid Mask and OFC Rescue ---
            ...
            # --- OFC Rescue: Protect anterior-inferior zone referenced to AC ---
            ...
            fslmaths "$T2_THR_MASK" -mul 0 -add 1 -roi 0 $DIM_X $AC_Y $SIZE_Y 0 $AC_Z 0 1 "${T1wFolder}/AC_Safe_Zone_tmp.nii.gz"
            fslmaths "$T2_THR_MASK" -add "${T1wFolder}/AC_Safe_Zone_tmp.nii.gz" -bin "$OFC_RESCUE_MASK"

            # Final merge: T1-thresholded mask constrained by T2-protected OFC region
            fslmaths "$T1_THR_MASK" -mul "$OFC_RESCUE_MASK" -bin "${T1wFolder}/final_tmp_mask.nii.gz"
            fslmaths "${T1wFolder}/final_tmp_mask.nii.gz" -ero -dilM "$MASK"
            cp -f "$MASK" "$FS_MASK"
                ...
                fslstats "${INPUT_BRAIN_T2}" -l 0.0001 -H 40 0 1000 | awk -v low="$AUTO_MIN_T2" -v high="$AUTO_MAX_T2" '
                    {
                        ...
                        content=""
                        for(i=0; i<$1/5000; i++){content=content mark}
                        print line content "|" $1
                    }' | tac | awk -F'|' 'found||$2>0{found=1; print $1}' | tac

                echo ""
                echo "--- T1w Visual Histogram (x: Out | o: In) ---"
                fslstats "${INPUT_BRAIN_T1}" -l 0.0001 -H 40 0 1000 | awk -v low="$AUTO_MIN_T1" -v high="$AUTO_MAX_T1" '
                    {
                        val=NR*25
                        line=sprintf("%5.0f: ", val)
                        mark=(val>high||val<low)?"x":"o"
    ...
    if applywarp \
        ...
        -o "${MNI_MASK}_new.nii.gz" >> "$SUBJ_LOG" 2>&1; then

        mv -f "${MNI_MASK}_new.nii.gz" "$MNI_MASK"

        for img in T1w_restore T1w T2w_restore T2w; do
            if [ -f "${AtlasSpaceFolder}/${img}.nii.gz" ]; then
                fslmaths "${AtlasSpaceFolder}/${img}.nii.gz" -mas "$MNI_MASK" "${AtlasSpaceFolder}/${img}_brain.nii.gz"
            fi
        done
...
log_info "---------------------------------------------------------------"
log_info "t2log-hybrid.sh: Hatano Skull Stripping Method v4.10 Complete."
